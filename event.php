...
if (isset($datasubmit))
{
   switch(true)
   {
      case ($datasubmit == "activate_event"):
         pem_cache_set("current_action", "activate");
         header('Location: ' . $pem_url . 'pem-admin/approve-event.php?did=' . $did . '&eid=' . $eid);
         break;
      case ($datasubmit == "edit-email"):
         if (isset($email_created) AND !empty($email_created)) pem_send_mail($email_created, $body, $subject, $from_address, $from_name);
         if (isset($email_contact1) AND !empty($email_contact1)) pem_send_mail($email_contact1, $body, $subject, $from_address, $from_name);
         if (isset($email_contact2) AND !empty($email_contact2)) pem_send_mail($email_contact2, $body, $subject, $from_address, $from_name);
         $message = __("Notification email sent.");
         break;
      case ($datasubmit == "deactivate_event"):
         if (!empty($did))
         {
         ...
         $message = __("Event deactivated.");
         break;
      case ($datasubmit == "deactivate_date"):
         $data = array("date_status" => "0");
         $where = array("id" => $did);
         pem_update_row("dates", $data, $where);
         $message = __("Date deactivated.");
         break;
      case ($datasubmit == "cancel_event"):
         if (!empty($did))
         {
            $data = array("date_cancelled" => "1");
            $where = array("id" => $did);
            pem_update_row("dates", $data, $where);
         }
         $data = array("entry_cancelled" => "1");
         $where = array("id" => $eid);
         pem_update_row("entries", $data, $where);
         $message = __("Event cancelled.");
         break;
      case ($datasubmit == "cancel_entry"):
         $data = array("entry_cancelled" => "1");
         $where = array("id" => $eid);
         pem_update_row("entries", $data, $where);
         $message = __("Event cancelled.");
         break;
      case ($datasubmit == "cancel_date"):
         $data = array("date_cancelled" => "1");
         $where = array("id" => $did);
         pem_update_row("dates", $data, $where);
         $message = __("Date cancelled.");
         break;
      case ($datasubmit == "uncancel_event"):
         if (!empty($did))
         {
            $data = array("date_cancelled" => "0");
            $where = array("id" => $did);
            pem_update_row("dates", $data, $where);
         }
         $data = array("entry_cancelled" => "0");
         $where = array("id" => $eid);
         pem_update_row("entries", $data, $where);
         $message = __("Event uncancelled.");
         break;
      case ($datasubmit == "uncancel_date"):
         $data = array("date_cancelled" => "0");
         $where = array("id" => $did);
         pem_update_row("dates", $data, $where);
         $message = __("Date uncancelled.");
         break;
      case ($datasubmit == "delete_event"):
         if (!empty($did))
         {
         ...
         $where = array("id" => $eid);
         pem_update_row("entries", $data, $where);
         //$message = __("Event deleted to recycle.");
         header("Location: $pem_url");
         break;
      case ($datasubmit == "delete_date"):
         $data = array("date_status" => "2");
         $where = array("id" => $did);
         pem_update_row("dates", $data, $where);
         $message = __("Date deleted to recycle.");
         break;
      case ($datasubmit == "copy_event"):
         header('Location: ' . $pem_url . 'add-event.php?did=' . $did);
         break;
      case ($datasubmit == "copy_date"):
         header('Location: ' . $pem_url . 'view.php?e=add');
         break;

      case ($datasubmit == "convert_to_event"):
         header('Location: ' . $pem_url . 'pem-admin/convert.php?did=' . $did . '&a=convertevent');
         break;
      case ($datasubmit == "convert_to_date"):
         header('Location: ' . $pem_url . 'pem-admin/convert.php?eid=' . $eid . '&a=convertdate');
         break;
   }
}
...
//      $entry_created_by = (empty($this_event["entry_created_by"])) ? "" : auth_get_user($this_event["entry_created_by"]);
//      $entry_edited_by = (empty($this_event["entry_edited_by"])) ? "" : auth_get_user($this_event["entry_edited_by"]);
//      $entry_approved_by = (empty($this_event["entry_approved_by"])) ? "" : auth_get_user($this_event["entry_approved_by"]);
      ...
      {
         $entry_created_by = (empty($this_event["entry_created_by"])) ? "" : auth_get_user($this_event["entry_created_by"]);
         $date_created_by = (empty($this_event["entry_created_by"])) ? "" : auth_get_user($this_event["date_created_by"]);
      ...
      {
         $entry_approved_by = (empty($this_event["entry_approved_by"])) ? "" : auth_get_user($this_event["entry_approved_by"]);
         $date_approved_by = (empty($this_event["entry_approved_by"])) ? "" : auth_get_user($this_event["date_approved_by"]);
      ...
      {
         $entry_edited_by = (empty($this_event["entry_edited_by"])) ? "" : auth_get_user($this_event["entry_edited_by"]);
         $date_edited_by = (empty($this_event["entry_edited_by"])) ? "" : auth_get_user($this_event["date_edited_by"]);
...
if (pem_user_authorized(array($access_type => "Add")))
{
...
if ($date_exists)
{
   echo '<div style="padding-top:10px; clear:both;">' . "\n";
   unset($controls);
   $controls[] = array("label" => __("Copy to New Event"), "onclick" => "action_submit('dataform', 'edit');");
   $controls[] = array("label" => __("Copy to New Date for This Event"), "onclick" => "action_submit('dataform', 'edit');");
   pem_controls($controls, true);
   echo '</div>' . "\n";
}
*/
...
// TODO to remove description label use the line below in place of the above
...
// TODO - to remove description label use this line instead of the above
//      pem_event_line($position, $date_description, "", true);
      ...
      case ($name == "date_category"):
         pem_event_line($position, $date_category_text, $label);
         break;
