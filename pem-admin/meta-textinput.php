...
if (isset($datasubmit))
{
   ...
   if (empty($input_label)) $error[] = __("Text Input Text cannot be empty.");

   $value_prep["input_label"] = $input_label;
   $value_prep["input_note"] = $input_note;

   switch (true)
   {
      case ($datasubmit == "new" AND isset($error)):
         $showaddform = false;
         pem_fieldset_begin(__("Add New Text Input Meta"));
      ...
      case ($datasubmit == "new"):
         ...
         $data = array("meta_name" => $meta_name, "meta_description" => $meta_description, "meta_type" => "textinput", "meta_parent" => $meta_parent, "internal_scheduled" => $internal_scheduled, "external_scheduled" => $external_scheduled, "internal_unscheduled" => $internal_unscheduled, "external_unscheduled" => $external_unscheduled, "value" => $value, "status" => $status);
         ...
         echo '<p><b>' . __("Text Input added.") . '</b></p>' . "\n";
         break;
      case ($datasubmit == "edit"):
         $showaddform = false;
         pem_fieldset_begin(__("Edit Text Input"));
      ...
      case ($datasubmit == "update"):
         ...
         echo '<p><b>' . __("Text Input updated.") . '</b></p>' . "\n";
         break;
      case ($datasubmit == "activate"):
         $data = array("status" => "1");
         $where = array("id" => $typeid);
         pem_update_row("meta", $data, $where);
         echo '<p><b>' . __("Text Input activated.") . '</b></p>' . "\n";
         break;
      case ($datasubmit == "deactivate"):
         $data = array("status" => "0");
         $where = array("id" => $typeid);
         pem_update_row("meta", $data, $where);
         echo '<p><b>' . __("Text Input deactivated.") . '</b></p>' . "\n";
         break;
      case ($datasubmit == "delete"):
         ...
         echo '<p><b>' . __("Text Input deleted to recycle.") . '</b></p>' . "\n";
         break;
   }
}

// list current items in the table
pem_fieldset_begin(__("Current Text Input Meta "));
echo '<p>' . __("Text Inputs add additional simple input fields to event forms.  They can provide additional descriptive information in views and be used as query options in dynamic reports.") . "</p>\n";
...
$where = array("meta_type" => "textinput");
...
function echo_form($id = "", $meta_name = "", $meta_description = "", $meta_parent = "", $internal_scheduled = "", $external_scheduled = "", $internal_unscheduled = "", $external_unscheduled = "", $status = "", $meta = "", $mode = "new", $error = "")
{
   ...
   $input_label_note = __("(This text will label the input field in submit and edit forms)");
   $input_note_note = __("(This text is displayed after the field to explain it to users)");

   pem_error_list($error);

   pem_form_begin(array("nameid" => "submitform", "action" => $PHP_SELF, "class" => "metainputform"));
   ...
   pem_field_label(array("default" => __("Text Input Label:"), "for" => "input_label", "style" => "width:auto;"));
   pem_text_input(array("nameid" => "input_label", "value" => $input_label, "size" => 30, "maxlength" => 40));
   pem_field_note(array("default" => $input_label_note));
   pem_field_label(array("default" => __("Text Input Note:"), "for" => "input_note", "style" => "width:auto;"));
   pem_text_input(array("nameid" => "input_note", "value" => $input_note, "size" => 30, "maxlength" => 40));
   pem_field_note(array("default" => $input_note_note));
