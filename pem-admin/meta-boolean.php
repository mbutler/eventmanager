<?php
/* ========================== FILE INFORMATION ================================= 
phxEventManager :: meta-boolean.php

============================================================================= */

$pagetitle = "Yes/No Select Meta Administration";
...
if (isset($datasubmit))
{
   ...
   if (empty($select_label)) $error[] = __("Select Label cannot be empty.");

   $value_prep["select_label"] = $select_label;
   $value_prep["yes_text"] = $yes_text;
   $value_prep["no_text"] = $no_text;
   $value_prep["select_note"] = $select_note;

   switch (true)
   {
      case ($datasubmit == "new" AND isset($error)):
         $showaddform = false;
         pem_fieldset_begin(__("Add New Yes/No Select Meta"));
      ...
      case ($datasubmit == "new"):
         ...
         $data = array("meta_name" => $meta_name, "meta_description" => $meta_description, "meta_type" => "boolean", "meta_parent" => $meta_parent, "internal_scheduled" => $internal_scheduled, "external_scheduled" => $external_scheduled, "internal_unscheduled" => $internal_unscheduled, "external_unscheduled" => $external_unscheduled, "value" => $value, "status" => $status);
         ...
         echo '<p><b>' . __("Yes/No Select added.") . '</b></p>' . "\n";
         break;
      case ($datasubmit == "edit"):
         $showaddform = false;
         pem_fieldset_begin(__("Edit Yes/No Select"));
      ...
      case ($datasubmit == "update"):
         ...
         echo '<p><b>' . __("Yes/No Select updated.") . '</b></p>' . "\n";
         break;
      case ($datasubmit == "activate"):
         $data = array("status" => "1");
         $where = array("id" => $typeid);
         pem_update_row("meta", $data, $where);
         echo '<p><b>' . __("Yes/No Select activated.") . '</b></p>' . "\n";
         break;
      case ($datasubmit == "deactivate"):
         $data = array("status" => "0");
         $where = array("id" => $typeid);
         pem_update_row("meta", $data, $where);
         echo '<p><b>' . __("Yes/No Select deactivated.") . '</b></p>' . "\n";
         break;
      case ($datasubmit == "delete"):
         ...
         echo '<p><b>' . __("Yes/No Select deleted to recycle.") . '</b></p>' . "\n";
         break;
   }
}

// list current items in the table
pem_fieldset_begin(__("Current Yes/No Select Meta"));
echo '<p>' . __('Yes/No Selects add additional drop-down select fields with the options "yes" and "no" to event forms.  They can provide additional descriptive information in views and be used as query options in dynamic reports.  There is no functional difference between this meta type and checkboxes; both are provided for visual preference.') . "</p>\n";
...
function echo_form($id = "", $meta_name = "", $meta_description = "", $meta_parent = "", $internal_scheduled = "", $external_scheduled = "", $internal_unscheduled = "", $external_unscheduled = "", $status = "", $meta = "", $mode = "new", $error = "")
{
   ...
   $select_label_note = __("(This text will appear next to the field in submit and edit forms)");
   ...
   pem_field_label(array("default" => __("Select Label:"), "for" => "select_label"));
   pem_text_input(array("nameid" => "select_label", "value" => $select_label, "size" => 40, "maxlength" => 64));
